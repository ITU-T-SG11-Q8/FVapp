    ...
    def run(self):
        while True:
            while self.running:
                ...
                cap.stop()
                print('# video input work......run......end')
...
class MainWindowClass(QMainWindow, form_class):
    def __init__(self, audio_queue):
        ...
        self.button_new_window.clicked.connect(self.new_dialog_func)
        ...
        self.new_dialog_ui = NewDialogClass()
        ...
        self.worker_webcam = WebcamWorker(self.preview, self.send_grm_queue, self.predictor, camera_index)
    ...
    def stop(self):
        self.worker_webcam.pause()
        self.worker_mic.pause()
        self.worker_speaker.pause()
    ...
    def remove_room(self):
        res = api.Removal(api.RemovalRequest(self.join_session.overlayId, self.join_session.ownerId, self.join_session.accessKey))
        if res.code is api.ResponseCode.Success:
            print("\nRemoval success.")
    ...
    def leave_room(self):
        res = api.Leave(api.LeaveRequest(overlayId=self.join_session.overlayId, peerId=self.peer_id, accessKey=self.join_session.accessKey))
        if res.code is api.ResponseCode.Success:
            print("\nLeave success.")
    ...
    def modify_information_room(self):
        ...
        modificationReq = api.ModificationRequest(overlayId=self.join_session.overlayId, ownerId=self.join_session.ownerId, adminKey=self.join_session.accessKey)
    ...
    def new_dialog_func(self):
        self.change_resolution()
        self.new_dialog_ui.set_view_status(True)
        self.new_dialog_ui.show()

    def change_resolution(self):
        index = self.comboBox_resolution.currentIndex()
        dialog_width = 1920
        dialog_heith = 1080
        if index == 2:
            dialog_width = 1600
        ...
        self.new_dialog_ui.change_size(dialog_width, dialog_heith)
...
class NewDialogClass(QtWidgets.QWidget):
    ...
    def change_size(self, p_width, p_height):
        self.m_width = p_width
        self.m_height = p_height
        self.setGeometry(0,0,self.m_width,self.m_height)
    ...
    def recv_image(self, p_image):
        ...
        if self.image is not None:
            ...
            if pixmap is not None:
                pixmap_resized = pixmap.scaledToWidth(self.label_view.width(), Qt.FastTransformation)
                if pixmap_resized is not None:
                    self.label_view.setPixmap(pixmap_resized)
