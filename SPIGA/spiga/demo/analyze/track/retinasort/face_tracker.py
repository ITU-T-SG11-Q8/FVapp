...
# My libs
import SPIGA.spiga.demo.analyze.track.retinasort.config as cfg
import SPIGA.spiga.demo.analyze.track.tracker as tracker
import SPIGA.spiga.demo.analyze.features.face as ft_face
...
class RetinaSortTracker(tracker.Tracker):
    ...
    def grm_encode_process_frame(self, image, tracked_obj):
        # tracked_obj = []
        features = self.detector.inference(image)
        bboxes = features['bbox']
        bboxes = self._code_bbox_idx(bboxes)
        ...
        return final_tracked_obj, features

    def grm_decode_process_frame(self, features, tracked_obj):
        # tracked_obj = []
        bboxes = features['bbox']
        bboxes = self._code_bbox_idx(bboxes)
        bboxes_id = self.associator.update(bboxes)
